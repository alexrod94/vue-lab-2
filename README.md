...
In this lab you'll continue exploring the basics on how to use Vue.js and will set the foundation for bigger, more complicated projects.
...
Yesterday you practiced showing a `string` inside a Vue component's template using moustache syntax (`{{}}`). Today, we've seen that strings are not the only things you can insert on your HTML using this technique.
...
Up for a harder challenge? Vue allows us to do pretty "magical" things in a simplified way. In this case, you'll learn how to change a CSS property dinamically using data binding.
...
- You'll have to create a method that changes the class or the property, and trigger it via a button click. We practiced that on yesterday's lab, so feel free to refer to it if you need any help.
